window.addEventListener('load', ()=>{
    ...
    box.innerHTML += `<h4 class="main-sw__subtitle">Zamień liczby z systemu Znak-Moduł (ZM) na dziesiętne (10)</h4>`
    for(let i = 0; i < 5; i++) {
        let index = zm(numbers[i])
        box.innerHTML += `
        <div class="excersize__box">
            <div>
                <p><span id="id${i}value">${index}</span><sub>(ZM)</sub></p>  
    ...
    box.innerHTML += `<h4 class="main-sw__subtitle">Zamień liczby dziesiętne (10) na liczby z systemu Znak-Moduł (ZM)</h4>`
    for(let i = 5; i < 10; i++) {
        box.innerHTML += `
        <div class="excersize__box">
            <div>
                <p><span id="id${i}value">${numbers[i]}</span><sub>(10)</sub></p>  
                <input type="number" id="id${i}input" placeholder="wartość (ZM)">
                <button id="id${i}btn" class="binary">Sprawdź</button><br>
            </div>
            <span id="id${i}text"></span>
        </div>
